...
lidar_pipeline_container_name = 'lidar_pipeline_container'
...
def generate_launch_description() -> LaunchDescription:
    ...
    aether_driver_share = get_package_share_directory('aether_driver')
    ...
    ekf_config_path = os.path.join(pkg_share, 'params', 'ekf.yaml')
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(pkg_share, 'map', 'house_map.yaml'),
    ...
    lidar_pipeline_container = Node(
        ...
        executable='component_container_mt',  # Multi-Threaded Executor for concurrency
        name=lidar_pipeline_container_name,
        namespace='',
        output='screen',
        # Crucial for performance: enabling Intra-Process Communication
    ...
    hardware_group = GroupAction(
        ...
        actions=[
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(pkg_share, 'launch', 'camera_launch.py'),
                ),
            ),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(pkg_share, 'launch', 'rplidar_launch.py'),
                ),
                launch_arguments={
                    'use_composition': 'True',
                    'container_name': lidar_pipeline_container_name,
                }.items(),
            ),
        ],
    )
    ...
    nav_bringup_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'slam_container_name': lidar_pipeline_container_name,
        }.items(),
    )
