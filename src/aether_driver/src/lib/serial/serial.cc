...
Serial::Serial(const string &port, uint32_t baudrate, aether_driver::Timeout timeout,
...
bool Serial::waitReadable() {
  aether_driver::Timeout timeout(pimpl_->getTimeout());
...
void Serial::setTimeout(aether_driver::Timeout &timeout) {
  pimpl_->setTimeout(timeout);
}

aether_driver::Timeout Serial::getTimeout() const { return pimpl_->getTimeout(); }

void Serial::setBaudrate(uint32_t baudrate) { pimpl_->setBaudrate(baudrate); }

uint32_t Serial::getBaudrate() const { return static_cast<uint32_t>(pimpl_->getBaudrate()); }
